...
<div class="card-form">
    <h3 style="margin-top: 0; color: #333;"><i class="fas fa-user-edit"></i> Edit Data Kelas</h3>
    <p style="color: #777; font-size: 13px; margin-bottom: 20px;">Silakan ubah data identitas kelas pada form di bawah ini.</p>
    ...
    <form action="<?= base_url('kelas/update/' . $kelas['id_kelas']) ?>" method="POST">
        ...
        <div class="form-group">
            <label>Tingkat</label>
            <input type="text" name="tingkat" class="form-control" value="<?= $kelas['tingkat'] ?>" required>
        </div>

        <div style="margin-top: 30px;">
            <button type="submit" class="btn-submit"><i class="fas fa-save"></i> Simpan Perubahan</button>
            <a href="<?= base_url('kelas') ?>" class="btn-cancel"><i class="fas fa-times"></i> Batal</a>
        </div>
    </form>
</div>
